...
with open(f'{cur_path}/info.csv', 'w', newline='') as info:
    fieldnames = ['full_name', 'first_name', 'last_name', 'seat_no', 'subject', 'paper_no', 'room'] # add student code / time remaining until exam starts
    ...
    for cl in myList:
        ...
        line = {"full_name": full_name,"first_name": first, "last_name": last, "seat_no": f'A{i}', "subject": subjects[random.randint(0,len(subjects)-1)], "paper_no": random.randint(1,3), "room": f'{random.randint(1,5)}0{random.randint(1,9)}'}
        i+=1
        print(line)
        csv_writer.writerow(line)
